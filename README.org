...
* Features
  ...
  - Provides real-time PTP metrics reporting via Unix sockets with JSON
    output, including clock summary statistics, synchronization events,
    and state changes with configurable(with [[https://github.com/codeplea/tinyexpr/tree/logic][tinyexpr]]) edge-triggered filtering.
